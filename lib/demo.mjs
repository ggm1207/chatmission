...
const missionPatterns = {
  ...
  friend_confidence: {
    validate: {
      attempt: /(비교|뒤처|초라|속상|불안|자신감|힘들|어떤|무엇|말해)/iu,
      strong:
        /(?=.*(?:비교|뒤처|초라|속상|불안|자신감|힘들))(?=.*(?:어떤\s*순간|무엇이|뭐가|가장|말해줄래|더\s*들어))/iu
    },
    separate: {
      attempt: /(점수|성적|친해|사실|가치|별로|부족|구분)/iu,
      strong:
        /(?=.*(?:점수|성적|친해|결과|사실))(?=.*(?:가치|전체|별로|부족|정해|뜻하))(?=.*(?:구분|분리|아니|하나로|전부))/iu
    },
    recover: {
      attempt: /(오답|문제|질문|인사|장점|오늘|해볼|선택)/iu,
      strong:
  ...
  group_work_bullying: {
    call_out: {
      attempt: /(지우|떠넘기|몰아|혼자|조용|예민|불공정|그만|부담)/iu,
      strong:
        /(?=.*(?:지우|조용하다고|거절|예민))(?=.*(?:떠넘기|몰아|혼자|압박))(?=.*(?:불공정|부담|그만|멈춰|나눠))/iu
    },
    support_jiwoo: {
      attempt: /(지우|혼자|괜찮|도움|필요|맡을|같이)/iu,
      strong:
        /(?=.*지우)(?=.*(?:혼자.{0,10}안|떠맡지|네\s*잘못\s*아니|괜찮|같이))(?=.*(?:어떤|무엇|맡을|도움|필요|할\s*수))/iu
    },
    redistribute: {
      attempt: /(역할|나누|분담|자료|정리|발표|선생님|기록|캡처)/iu,
      strong:
...
const weakReplies = {
  ...
  friend_confidence: [
    "괜찮다는 말만 들으면 오히려 내가 왜 힘든지 설명하기 어려워. 비교 때문에 어떤 점이 아픈지 먼저 물어봐줘.",
    "나를 칭찬만 하기보다 점수나 관계 걱정이 곧 내 가치 전체를 뜻하는 건 아닌지 같이 구분해줘.",
    "큰 결심은 부담돼. 오늘 해볼 수 있는 작은 행동을 내가 고를 수 있게 도와줘."
  ],
  boundary: [
  ...
  nickname_boundary: [
    "나를 몰아붙이기보다 내가 반복해서 한 행동을 구체적으로 말해줘.",
    "장난이라고만 생각했어. 그 별명이 너에게 어떤 감정과 영향을 줬는지 말해줘.",
    "앞으로 뭘 멈추고 단체방에서 무엇을 바로잡아야 하는지 구체적으로 요청해줘."
  ],
  group_bullying: [
  ...
  group_work_bullying: [
    "준서를 공격하기보다 지우에게 일이 몰리고 압박받는 행동이 왜 불공정한지 말해줘.",
    "지우에게 직접 말해서 혼자 떠맡지 않아도 된다고 알려주고, 어떤 도움이 필요한지 물어봐줘.",
    "역할을 어떻게 나눌지와 반복될 때 어떤 기록을 남겨 누구에게 알릴지 구체적으로 말해줘."
  ]
};
...
function demoMessages(scenario, targetIndex, text) {
  ...
  const match = names
    ? String(text).match(new RegExp(`^(${names}):\\s*(.+)$`, "u"))
    : null;
  ...
  return [
    {
      speaker: speakers[Math.min(targetIndex, speakers.length - 1)] ?? speakers[0] ?? "",
      text
    }
  ];
}
